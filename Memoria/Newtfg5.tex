...
def tipifylist(list_to_tipify):
	...
	desv = [math.sqrt(x_desv/(number_of_data-1)),
	math.sqrt(y_desv/(number_of_data-1)), 
	math.sqrt(z_desv/(number_of_data-1))]
	print(desv)

	for row in list_to_tipify:
		row = [(row[0]-media[0])/desv[0], 
				(row[1]-media[1])/desv[1], 
				(row[2]-media[2])/desv[2]]
 ...
 *
 * The device will setup a listener to receive messages from the wear device 
 and display them to the screen.
 *
 * It also setups up a button, so it can send a message to the wear device, 
 the wear device will auto
 * response to the message.   This code does not auto response, otherwise 
 ...
 * in the gradle (both wear and mobile).  Also the applicationId MUST be 
 the same in both files as well
